...
ggplot(nest_boxes_clean, # starting with the clean data frame
  ...
  geom_line(data = model_prediction, # setting the new data frame with predicitons from model 1 (distance and season)
            ...
            size = 1, inherit.aes = FALSE) + # setting the size of the dot and removing parameters from geom ribbon call
  theme_bw() + # using black and white ggplot theme
...
Swift Parrot occupancy was predicted to be higher at a lower edge distance and during the 2016 season rather than the 2019 (Figure 1). With an increase from 2016 to 2019, the odds of a box being occupied by a swift parrot fall by a factor of 77% (95% CI: [0.62, 0.96], p = 0.022, $\alpha$ = 0.05); With each meter increase in edge distance, the odds of occupancy fall by a factor of 99.8% (95% CI: [0.997, 0.999], p < 0.001, $\alpha$ = 0.05). 
